  ...
  connection
    ...
    .catch((err) => {
      ...
      if (
        ...
        console.log(
          "Authentication required - not retrying SignalR connection"
        );
        return;
      }
...
function setupSignalREventHandlers() {
  ...
  connection.on("ReceiveRoomMessage", (message) => {
    ...
    if (!message.isOwnMessage) {
      showSystemNotification(
        `New message in ${message.roomName || selectedRoom}`,
        `${message.sender.name}: ${message.content}`
      );
    }
  });
  ...
  connection.on(
    "ReceiveCall",
    async ({ callerId, callerName, video, offer }) => {
      // Lưu lại thông tin offer, chờ accept
      window.lastCallOffer = { offer, callerId, video };
            ...
            <div class="d-flex align-items-center">
                ...
                <div class="dropdown">
                    ...
                    <ul class="dropdown-menu">
                        <li><a class="dropdown-item" href="#" onclick="event.stopPropagation(); removeFriend('${
                          friend.friendshipId
                        }')">Remove Friend</a></li>
                    </ul>
                </div>
...
function displayPendingRequests(requests) {
  ...
  if (requestsList) {
    ...
    if (requests.length === 0) {
      requestsList.innerHTML =
        '<div class="p-3 text-center text-muted">No pending requests</div>';
...
function respondToFriendRequest(friendshipId, accept) {
  fetch(
    `/Chat/RespondToFriendRequest?friendshipId=${friendshipId}&accept=${accept}`,
    {
      method: "POST",
    }
  )
...
function showRoomInviteNotification(invite) {
  showNotification(
    `${invite.inviterName} invited you to join room: ${invite.roomName}`
  );
...
function sendRoomMessage() {
  ...
  if (content) {
    ...
    connection
      .invoke("SendRoomMessage", selectedRoom, content)
...
function selectRoom(roomName) {
  ...
  const roomElement = document.querySelector(
    `.room-item[data-room-name="${roomName}"]`
  );
  if (roomElement) {
    ...
    fetch("/Chat/GetAllRooms")
      ...
      .then((rooms) => {
        ...
        if (roomsList) {
          ...
          if (
            rooms.length > 0 &&
            document.getElementById("auto-join-room")?.value === "true"
          ) {
...
function loadUsersInRoom(roomName) {
  ...
  connection
    .invoke("GetUsersInRoom", roomName)
    .then((users) => {
...
function markMessageAsRead(messageId) {
  connection
    .invoke("MarkMessageAsRead", messageId)
...
function displayTypingIndicator(userId) {
  ...
  const userName =
    document.querySelector(`#user-${userId} .contact-name`)?.textContent ||
    "Someone";
...
function displayRoomTypingIndicator(roomName, userId) {
  ...
  // Get user name
  const userName =
    document.querySelector(`#room-user-${userId} .user-name`)?.textContent ||
    "Someone";
...
function updateGroupMemberStatus(user) {
  // Update in room users list if present
  ...
  if (selectedGroupId) {
    ...
    if (groupUsersList) {
      const groupUserElement = groupUsersList.querySelector(
        `[data-user-id="${user.userId}"]`
      );
      if (groupUserElement) {
        const statusIndicator =
          groupUserElement.querySelector(".status-indicator");
        if (statusIndicator) {
          statusIndicator.classList.remove("online", "offline");
          statusIndicator.classList.add(user.isOnline ? "online" : "offline");
        }
        ...
        const statusTextElement =
          groupUserElement.querySelector(".status-text");
        if (statusTextElement) {
          let statusText = user.isOnline ? "Online" : "Offline";
          statusTextElement.textContent = statusText;
        }
...
function updateMessageStatus(messageId, status) {
  const statusElement = document.querySelector(
    `.message-content[data-message-id="${messageId}"] .message-status`
  );
  if (statusElement) {
...
function handleTyping() {
  if (selectedUserId) {
    connection
      .invoke("NotifyTyping", selectedUserId)
      .catch((err) =>
        console.error("Error sending typing notification: ", err)
      );
  } else if (selectedGroupId) {
    connection
      .invoke("NotifyGroupTyping", selectedGroupId)
      .catch((err) =>
        console.error("Error sending group typing notification: ", err)
      );
  } else if (selectedRoom) {
    connection
      .invoke("NotifyRoomTyping", selectedRoom)
      .catch((err) =>
        console.error("Error sending room typing notification: ", err)
      );
  }
}
...
function searchUsers(displayName) {
  ...
  fetch(
    `/Chat/SearchUsers?displayName=${encodeURIComponent(displayName.trim())}`
  )
    .then((response) => response.json())
    .then((users) => {
...
function searchUsersForInvite(query) {
  ...
  fetch(
    `/Chat/SearchUsersForGroupInvite?groupId=${groupId}&query=${encodeURIComponent(
      query.trim()
    )}`
  )
    .then((response) => response.json())
...
function displayInviteSearchResults(users) {
  ...
  users.forEach((user) => {
    ...
    userElement.innerHTML = `
            ...
            <button class="btn btn-sm btn-primary" onclick="addUserToGroup(${
              document.getElementById("invite-group-id").value
            }, '${user.userId}')">
                Add to Group
            </button>
        `;
  ...
  fetch("/Chat/AddUserToGroup", {
    ...
    .then((response) => {
      if (response.ok) {
        ...
        // Close the modal
        const modal = bootstrap.Modal.getInstance(
          document.getElementById("inviteToGroupModal")
        );
        if (modal) modal.hide();
      } else {
        response.text().then((text) => showErrorMessage(text));
...
function searchUsersForRoomInvite(query) {
  ...
  fetch(
    `/Chat/SearchUsersForRoomInvite?roomName=${encodeURIComponent(
      roomName
...
function displayRoomInviteSearchResults(users) {
  ...
  users.forEach((user) => {
    ...
    userElement.innerHTML = `
            ...
            <button class="btn btn-sm btn-primary" onclick="inviteUserToRoom('${
              document.getElementById("invite-room-name").value
            }', '${user.userId}')">
                Invite to Room
            </button>
        `;
  ...
  fetch("/Chat/InviteUserToRoom", {
    ...
    .then((response) => {
      if (response.ok) {
        ...
        // Close the modal
        const modal = bootstrap.Modal.getInstance(
          document.getElementById("inviteToRoomModal")
        );
        if (modal) modal.hide();
      } else {
        response.text().then((text) => showErrorMessage(text));
...
function initializeEmojiPicker() {
  ...
  if (!document.querySelector("emoji-picker")) {
    ...
    emojiPicker.addEventListener("emoji-click", (event) => {
      ...
      const newCursorPos = selectionStart + event.detail.unicode.length;
  ...
  function positionEmojiPicker() {
    ...
    if (emojiButton && emojiPickerContainer) {
      ...
      emojiPickerContainer.style.bottom =
        window.innerHeight - chatFooterRect.top + "px";
      emojiPickerContainer.style.left = buttonRect.left + "px";
    }
  }

  // Toggle emoji picker when the button is clicked
  const emojiButton = document.getElementById("emoji-picker-button");
  const emojiPickerContainer = document.getElementById(
    "emoji-picker-container"
  );
...
function closeEmojiPickerOnClickOutside(event) {
  ...
  // Check if the click was outside both the picker and the button
  if (
...
async function startCall(video = false) {
  window.isCurrentCallVideo = video;
  ...
  peerConnection.onicecandidate = (event) => {
    if (event.candidate) {
      if (!selectedUserId) {
        console.error("selectedUserId is null! Cannot send ICE.");
        return;
      }
      connection.invoke("SendIceCandidate", selectedUserId, event.candidate);
    }
  };
  ...
  // Chỉ show call-controls và UI video nếu là video
  if (video) {
    ...
    showCallControls(false);
    document.getElementById("video-call-container").style.display = "none";
    document.getElementById("audio-call-overlay").style.display = "flex";
    document.getElementById("localVideo").style.display = "none";
    document.getElementById("remoteVideo").style.display = "none";
    document.getElementById("audio-call-username").innerText =
      lastCallUserName || "Đang kết nối...";
...
function showIncomingCallPopup({ callerId, callerName, video }) {
  ...
  nameEl.textContent = callerName || "Người lạ";
  typeEl.textContent = video ? "Cuộc gọi Video" : "Cuộc gọi Audio";
  popup.style.display = "flex";

  // Accept
  document.getElementById("call-accept-btn").onclick = async function () {
    popup.style.display = "none";
    showCallUI(video, callerName); // <- LỆNH QUAN TRỌNG: Phải truyền đúng biến video!!!
    await acceptCall(window.lastCallOffer.callerId, video);
  };
  // Reject
...
async function acceptCall(fromUserId, video) {
  localStream = await navigator.mediaDevices.getUserMedia({
  ...
  lastCallUserName =
    document.getElementById("call-popup-caller-name")?.textContent || "";
  peerConnection.ontrack = (event) => {
    remoteStream = event.streams[0];
    showRemoteStream(remoteStream);
  };
  lastCallPeerId = selectedUserId || fromUserId;
  peerConnection.onicecandidate = (event) => {
    if (event.candidate) {
      if (!fromUserId) {
        console.error("fromUserId is null in acceptCall! Cannot send ICE.");
        return;
      }
      connection.invoke("SendIceCandidate", fromUserId, event.candidate);
    }
  };

  // Dùng remote offer đã lưu từ handleReceiveOffer
  if (window.lastCallOffer) {
    await peerConnection.setRemoteDescription(
      new RTCSessionDescription(window.lastCallOffer.offer)
    );
    const answer = await peerConnection.createAnswer();
    await peerConnection.setLocalDescription(answer);
    connection.invoke("AnswerCall", fromUserId, answer);

    // Hiển thị UI call đúng loại
    showCallUI(video, lastCallUserName);
  }
}
...
function toggleMic(e) {
  ...
  // Lấy đúng nút bấm (event target)
  let icon = e?.currentTarget?.querySelector("i");
  if (!icon) {
    // Fallback: lấy cả 2, đổi cả hai luôn cho chắc
    document
      .querySelectorAll("#audio-toggle-mic-btn i, #toggle-mic-btn i")
      .forEach((i) => {
        i.className = micEnabled ? "bi bi-mic-fill" : "bi bi-mic-mute-fill";
      });
  } else {
    icon.className = micEnabled ? "bi bi-mic-fill" : "bi bi-mic-mute-fill";
  }
}
...
function endCall(fromRemote = false) {
  ...
  if (lastCallType === "outgoing" && lastCallUserName) {
    msg = `Bạn đã gọi cho <b>${lastCallUserName}</b> lúc ${time}`;
  } else if (lastCallType === "incoming" && lastCallUserName) {
    msg = `<b>${lastCallUserName}</b> đã gọi cho bạn lúc ${time}`;
  } else {
    msg = "Cuộc gọi đã kết thúc";
  }
  displaySystemMessage(msg);

  // ĐÃ THÊM: ngăn gửi lặp khi end từ remote (bên kia)
  if (!fromRemote && lastCallPeerId) {
    if (!isCallEnding) {
      isCallEnding = true;
      console.log(
        "[endCall] Invoke EndCall to: ",
        lastCallPeerId,
        lastCallType,
        lastCallUserName
      );
  ...
  } else if (!fromRemote && !lastCallPeerId) {
    console.warn(
      "[endCall] lastCallPeerId null, không gửi EndCall lên server!"
    );
  }
  lastCallType = null;
...
function displaySystemMessage(content) {
  ...
  systemMsg.innerHTML = `<span>${content}</span>`;
  messageList.appendChild(systemMsg);
  scrollToBottom();
}

function showCallControls(show = true) {
...
function showCallUI(isVideo, userName) {
  if (isVideo) {
    document.getElementById("video-call-container").style.display = "block";
    document.getElementById("audio-call-overlay").style.display = "none";
    document.getElementById("localVideo").style.display = "";
...
function makePopupDraggable(popupSelector, dragHandleSelector) {
  const popup = document.querySelector(popupSelector);
  const handle = popup.querySelector(dragHandleSelector);
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Handle room info toggle
  document
    .querySelector(".room-info .btn-close")
    ?.addEventListener("click", function () {
      toggleRoomInfo();
    });
  ...
  document
    .getElementById("toggle-mic-btn")
    ?.addEventListener("click", toggleMic);
  document
    .getElementById("toggle-cam-btn")
